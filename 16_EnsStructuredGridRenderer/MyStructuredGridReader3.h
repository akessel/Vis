...
class MyStructuredGridReader3
{
	...
	public:
		MyStructuredGridReader3(string filename, int nRad, int nLat, int nLong,
					...
					for(int k = 0; k < this->nRad; k++)
					{
						if(i != this->nLong)
						{
							...
							*/
							...
							if(i == 0) printf("[%d, (%g, %g, %g), %g]\t", count, x, y, z, value);
						}
						else // i == this->nLong // ghost rows; stitching up the discontinuity
						{
							...
							points->InsertPoint(count, xyz[0], xyz[1], xyz[2]);
							scalars->InsertValue(count, value);
							
							if(j == 0 && k == 0) printf("\n\n");
							
							printf("[%d, (%g, %g, %g), %g]\t", pseudoCount, xyz[0], xyz[1], xyz[2], value);
						}
